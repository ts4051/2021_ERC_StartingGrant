...
\todo{QG today. LIV, photons etc. Gravitational waves. }
...
\todo{More about the exising IceCUeb data, and the re-calibration}
...
This work will be by far the most sensitive and comprehensive search for energy-suppressed neutrino decoherence ever performed. These results will achieve sensitivity to the naturally expected scale of quantum gravity effects for a number of the models tested \todo{More specific?}, giving genuine hope of the first quantum gravity signal detection, a truly revolutionary result. A null result in these models will reject all but the most energy-suppressed Planck scale decoherence scenarios, or indicate that their effects are much weaker than predicted. Either way these results will be invaluable in informing the theoretical development of quantum gravity. For the publication of the results I wil target the high impact journal \textit{Nature} (which typically accepts those rare results achieving sensitivity to Planck scale physics) since it is guaranteed to either detect or rule out some models. \\
...
%     \centering
%     % \vspace{-7pt}
%     \includegraphics[trim=2.0cm 0.0cm 0.0cm 0.0cm, clip=true, width=\linewidth]{images/blazar.jpg}
%     % \end{subfigure}
%     \caption{Neutrinos and other particles travelling from a blazar to the Earth.}
%     % \vspace{-7pt}
%     \label{fig:blazar}
% \end{wrapfigure}

% IceCube also observes a flux of neutrinos of astrophysical origin. I have shown that despite the vast travel distances, astrophysical neutrinos unfortunately have limited sensitivity to decoherence, as any oscillation effects are typically washed out~\cite{PhysRevD.102.115003}. However, the fact that they are observed at all strongly constrains scenarios where neutrinos are lost as they travel, including absorption by the $\nu$-VBH interactions considered in this work, or neutrino decay.

% I will exploit the recent observations of both neutrino and gamma-ray emission from the blazar TXS 0506+056~\cite{eaat1378} (a super-massive black hole accreting its host galaxy and radiating powerful particle jets), which have travelled a colossal $\sim$1\,Gpc, to strongly constrain these `neutrino loss' scenarios (as well as scenarios where neutrinos are scattered from the line-of-sight, including Dark Matter scattering), yielding limits of the neutrino mean free path. I have shown that this novel analysis is \textbf{sensitive to physics orders of magnitude weaker than even the atmospheric neutrino analyses in this work} (although only for a sub-set of the models considered), and thus this work can place unprecedented constraints on Planck scale physics (even if suppressed by 4 powers of energy). I will again target a high profile journal with this work.

% \noindent \textbf{[Task 5]} Determine both neutrino and photon flux estimate upper limits for the flaring blazar, with support of world class expertise in multi-messenger astrophysics at the Niels Bohr Institue. \\
% %
% \noindent \textbf{[Task 6]} Model various propagation scenarios and combine with estimated source vs observed particle fluxes to determine limits on the neutrino mean free paths. 

% Analysis A will be the \textbf{most comprehensive analysis of neutrino decoherence ever performed}, searching the GeV-PeV atmospheric neutrino flux detected by IceCube for signs of energy-dependent damping of neutrino oscillations (e.g. Fig. \ref{fig:decoh_oscillograms}) resulting from decoherence over distances up of up $\sim$12,700\,km (the Earth's diameter). This broad energy range maximises the sensitivity to many potential sources of decoherence. I will test the follow scenarios:

% \begin{itemize}
%     \itemsep-0.3em 
%     \item $\nu$-VBH interactions using my models, constraining the neutrino mean free path.
%     \item Lightcone fluctuations using my model, constraining their scale.
%     \item CPT-V decoherence.
%     \item A generalised decoherence search, independent of the underlying physics.
% \end{itemize}

% \noindent \textbf{[Task A.1]} Develop (for the first time) an atmospheric neutrino data sample spanning the full energy reach of IceCube (GeV to PeV). This will require 

% \todo{Synergies with Jason/Markus}

% % \noindent \textbf{[Task A.1]} Develop high fidelity models of the new optical sensors and related backgrounds, verified against detector data. I  lead simulation development in my role as `IceCube Upgrade simulation manager', where I am the named responsible person to the US National Science Foundation for delivering all simulations for this \$40 million project. \\
% % \noindent \textbf{[Task A.2]} Production of Monte Carlo simulation datasets of neutrino interactions and backgrounds in the upgraded IceCube detector, integrating recent developments in neutrino modelling software and interaction theoretical calculations to produce consistent modelling spanning GeV-PeV energies.\\
% % \noindent \textbf{[Task A.3]} Identify IceCube neutrino events amongst an overwhelming atmospheric muon background. I have developed state-of-the-art machine learning methods for identifying GeV-scale events as part of my previous oscillation measurements, which I will extend to exploit the advanced new sensors of the upgraded detector and to identify km-scale high-energy neutrino interactions. \\
% % \noindent \textbf{[Task A.4]} Extend my decoherence modelling to include CPT violating operators.\\
% % \noindent \textbf{[Task A.5]} Develop techniques to distinguish (anti)neutrinos  using reconstructed momentum transfer and tagging delayed light from neutron capture to increase sensitivity to CPT violating decoherence.

% This work will yield multiple publications covering the various scenarios tested, and given their nearly unparalleled Planck-scale reach I will target high impact journals such as Nature or Science.
