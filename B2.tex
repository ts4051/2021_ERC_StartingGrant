...
\textbf{Risk/Gain:} The cross section modelling used for $<$100 GeV neutrinos in \texttt{GENIE} uses older \textit{leading order} (LO) PDFs (GRV98) than the recent high energy extension (as they are available with low $Q^2$ Bodek-Yang~\cite{Bodek:2010km} corrections required at this energy range\todo{Am I sure about this?}), with the main difference being the relative contributions of sea vs. valence light quarks. I am in contact with the developers of the recent \texttt{GENIE} extension to coordinate an update to this, but if not available by the time of this project I will implement a transition region interpolating the cross section, inelasticity and the 4-mometa of the outgoing particles between the two regimes, with associated systematic uncertainty. I will also integrate \texttt{GENIE} tuning data provided by neutrino accelerator cross section measurements~\cite{Stowell:2019zsh, Acero:2020eit}, which may at least partially correct for the limitations of the LO \texttt{GENIE} cross section calculations.
...
TODO Coordinating with GENIE authors for sys uncertainty (including inetrgating into IceCube analysis software)
